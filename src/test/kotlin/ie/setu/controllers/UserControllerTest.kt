...
class UserControllerTest {
    ...
    fun ensureUserDoesNotExist() {
        val retrieveUserResponse = requests.retrieveUserByEmail(VALID_EMAIL, jwtToken)
    ...
    inner class ReadUsers {
        ...
        fun `get all users from the database returns 200 or 404 response`() {
            val response = requests.retrieveUsers(jwtToken)
        ...
        fun `get user by id when user does not exist returns 404 response`() {
            ...
            val retrieveResponse = requests.retrieveUserById(id, jwtToken)
        ...
        fun `get user by email when user does not exist returns 404 response`() {
            ...
            val retrieveResponse = requests.retrieveUserByEmail(NONE_EXISTING_EMAIL, jwtToken)
        ...
        fun `getting a user by id when id exists, returns a 200 response`() {
            ...
            val addResponse = requests.addUser(VALID_NAME, VALID_EMAIL, VALID_PASSWORD, jwtToken)
            val addedUser: User = jsonToObject(addResponse.body.toString())

            // Assert - retrieve the added user from the database and verify return code
            val retrieveResponse = requests.retrieveUserById(addedUser.id, jwtToken)
            assertEquals(200, retrieveResponse.status)

            // After - restore the db to previous state by deleting the added user
            val deleteUserResponse = requests.deleteUser(addedUser.id, jwtToken)
        ...
        fun `getting a user by email when email exists, returns a 200 response`() {
            // Arrange - add the user
            requests.addUser(VALID_NAME, VALID_EMAIL, VALID_PASSWORD, jwtToken)

            // Assert - retrieve the added user from the database and verify return code
            val retrieveResponse = requests.retrieveUserByEmail(VALID_EMAIL, jwtToken)
            assertEquals(200, retrieveResponse.status)

            // After - restore the db to previous state by deleting the added user
            val retrievedUser: User = jsonToObject(retrieveResponse.body.toString())
            val deleteUserResponse = requests.deleteUser(retrievedUser.id, jwtToken)
            assertEquals(204, deleteUserResponse.status)
        }
    }
    ...
    inner class UpdateUsers {
        ...
        fun `updating a user when it exists, returns a 204 response`() {
            ...
            val addedResponse = requests.addUser(VALID_NAME, VALID_EMAIL, VALID_PASSWORD, jwtToken)
            ...
            assertEquals(204, requests.updateUser(addedUser.id, UPDATED_NAME, UPDATED_EMAIL, VALID_PASSWORD, jwtToken).status)

            // Act & Assert - retrieve updated user and assert details are correct
            val updatedUserResponse = requests.retrieveUserById(addedUser.id, jwtToken)
    ...
    inner class DeleteUsers {
        ...
        fun `deleting a user when it doesn't exist, returns a 404 response`() {
            ...
            assertEquals(404, requests.deleteUser(Integer.MIN_VALUE, jwtToken).status)
        }
        ...
        fun `deleting a user when it exists, returns a 204 response`() {
            ...
            assertEquals(204, requests.deleteUser(addedUser.id, jwtToken).status)

            // Act & Assert - attempt to retrieve the deleted user --> 404 response
            assertEquals(404, requests.retrieveUserById(addedUser.id, jwtToken).status)
        }
    }
}
